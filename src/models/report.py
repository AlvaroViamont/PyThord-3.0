...
class PDF(FPDF):
    ...
    def _first_page(self):
        ...
        self.cell(0, 5, f'Cadencia: {self.patient.cadence} pasos/minuto', 0, 1, 'L')
        self.set_x(13.5)
        self.cell(0, 5, f'Tiempo Promedio (D): {self.patient.right_time} s', 0, 1, 'L')
        self.set_x(13.5)
        self.cell(0, 5, f'Tiempo Promedio (I): {self.patient.left_time} s', 0, 1, 'L')
        
        self.set_xy(95, 68.9)
        self.cell(0, 5, f'Velocidad: {self.patient.speed} m/s', 0, 1, 'L')
        self.set_x(95)
        self.cell(0, 5, f'Longitud Zancada Derecha: {self.patient.mean_distancer} m', 0, 1, 'L')
        self.set_x(95)
    ...
    def _create_rcomparative_plot_analytic_section(self, x_column, y1_column, y2_column, title, xlabel, ylabel, p1label, p2label, ri, li, rd, ld):
        self.set_font('Arial', 'B', 16)
        self.set_text_color(50, 55, 59)
        self.ln(10)
        self.cell(0, 0, f'{title}', 0, 1, 'C', True)
        ...
        with plt.style.context(custom_style_2):
            ...
            plt.plot(x_column, y1_column, label=p1label)
            plt.plot(x_column, y2_column, label=p2label)
            plt.plot(ri*10, rd, "o", color='#FAB860')
            plt.plot(li*10, ld, "o", color='#FAB860')
            plt.xlabel(xlabel)
            plt.ylabel(ylabel)
            x_min = 0
            x_max = self.data.data_size*10
    ...
    def build(self):
        self._first_page()
        self.add_page()
        self._create_rcomparative_plot_analytic_section(self.data.stride_raw_data['RDTime(ms)'], self.data.stride_raw_data['RDSagital'], self.data.stride_raw_data['RISagital'], 'Rodilla en el plano Sagital', 'Tiempo (ms)', 'RD vs RI (grados)', 'RD', 'RI', self.data.rdata_ipeaks, self.data.ldata_ipeaks, self.data.rdata_peaks, self.data.ldata_peaks)
        self._create_comparative_plot_analytic_section(self.data.stride_raw_data['RDTime(ms)'], self.data.stride_raw_data['RDFrontal'], self.data.stride_raw_data['RIFrontal'], 'Rodilla en el plano Frontal', 'Tiempo (ms)', 'RD vs RI (grados)', 'RD', 'RI')
        self.add_page()
        self._create_comparative_plot_analytic_section(self.data.stride_raw_data['CDTime(ms)'], self.data.stride_raw_data['CDSagital'], self.data.stride_raw_data['CISagital'], 'Caderas en el plano Sagital', 'Tiempo (ms)', 'CD vs CI (grados)', 'CD', 'CI')
        self._create_comparative_plot_analytic_section(self.data.stride_raw_data['CDTime(ms)'], self.data.stride_raw_data['CDFrontal'], self.data.stride_raw_data['CIFrontal'], 'Caderas en el plano Frontal', 'Tiempo (ms)', 'CD vs CI (grados)', 'CD', 'CI')
        '''self.add_page()
        self._create_comparative_plot_analytic_section(self.data_dict['Time(ms)'], self.data_dict['TCDSagital'], self.data_dict['TCISagital'], 'Caderas en el plano Sagital data Transformada', 'Tiempo (ms)', 'CD vs CI (grados)')
        self._create_comparative_plot_analytic_section(self.data_dict['Time(ms)'], self.data_dict['TCDFrontal'], self.data_dict['TCIFrontal'], 'Caderas en el plano Frontal data Transformada', 'Tiempo (ms)', 'CD vs CI (grados)')
        self.add_page()
        self._create_comparative_plot_analytic_section(self.data_dict['Time(ms)'], self.data_dict['TRDSagital'], self.data_dict['TRISagital'], 'Rodilla en el plano Sagital data Transformada', 'Tiempo (ms)', 'RD vs RI (grados)')
        self._create_comparative_plot_analytic_section(self.data_dict['Time(ms)'], self.data_dict['TRDFrontal'], self.data_dict['TRIFrontal'], 'Rodilla en el plano Frontal data Transformada', 'Tiempo (ms)', 'RD vs RI (grados)')
        self._create_table_section(self.data_dict, 'Sagital')
        self._create_table_section(self.data_dict, 'Frontal')'''
        plt.rcParams.update(custom_style)
        pdf_path = get_patient_doc_file(str(self.patient.ci), ''.join((self.report_name, '.pdf')))
